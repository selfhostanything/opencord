...
describe('native mobile media configuration', () => {
  ...
  it('includes iOS native project permission purpose strings and LiveKit setup', () => {
    ...
    expect(infoPlist).toContain('group.com.opencord.screenshare')
    expect(infoPlist).toContain('RTCScreenSharingExtension')
    expect(infoPlist).toContain('com.opencord.ScreenShareExtension')
  ...
  it('includes iOS ReplayKit screen-share extension wiring', () => {
    const project = readMobileFile('ios/OpenCord.xcodeproj/project.pbxproj')
    const appEntitlements = readMobileFile('ios/OpenCord/OpenCord.entitlements')
    const extensionInfo = readMobileFile('ios/OpenCordScreenShareExtension/Info.plist')
    const extensionEntitlements = readMobileFile(
      'ios/OpenCordScreenShareExtension/OpenCordScreenShareExtension.entitlements',
    )
    const sampleHandler = readMobileFile('ios/OpenCordScreenShareExtension/SampleHandler.swift')

    expect(project).toContain('ScreenShareExtension.appex')
    expect(project).toContain('Embed App Extensions')
    expect(project).toContain('OpenCordScreenShareExtension/Info.plist')
    expect(project).toContain('OpenCordScreenShareExtension/OpenCordScreenShareExtension.entitlements')
    expect(appEntitlements).toContain('group.com.opencord.screenshare')
    ...
    expect(extensionEntitlements).toContain('group.com.opencord.screenshare')
    expect(sampleHandler).toContain('final class SampleHandler: RPBroadcastSampleHandler')
    expect(sampleHandler).toContain('group.com.opencord.screenshare')
    expect(sampleHandler).toContain('rtc_SSFD')
    expect(sampleHandler).toContain('Buffer-Width')
    expect(sampleHandler).toContain('Buffer-Height')
  })
})
